...
#define DEBOUNCE_DELAY 20  // 典型值为10-50ms / Typical value: 10-50ms
...
KeyEvent Key_Scan(Key_t* key, uint32_t currentTime, uint32_t longPressThreshold) {
    // 读取引脚电平：0表示按下，1表示释放 / Read pin level: 0=pressed, 1=released
    uint8_t isPressed = 0;
    ...
    switch (key->state) {
        ...
        case KEY_STATE_DEBOUNCE:
            // 消抖时间到达，检查稳定状态 / Check stable state after debounce delay
            if (currentTime - key->debounceTime >= DEBOUNCE_DELAY) {
                if (isPressed) {
                    ...
                    key->state = KEY_STATE_RELEASED;   // 误触发 / False trigger due to bounce
                }
            }
            break;
...
{
    int16_t LongPressThreshold = 700;// 按键扫描时的长按阈值：500ms    Threshold for long press during key scanning: 500ms
    static uint32_t lastTick = 0;  // 初始时间  initial time
    uint32_t currentTick = Get_Time();
    static int task_flag = 1;

    // 每10ms检查一次按键 - Check key every 10ms
