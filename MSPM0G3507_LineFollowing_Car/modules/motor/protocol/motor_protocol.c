...
{
    int length = snprintf((char *)send_buff, sizeof(send_buff), "$mtype:%d#", data);
    if (length <= 0 || length >= (int)sizeof(send_buff)) return false;
...
{
    int length = snprintf((char *)send_buff, sizeof(send_buff), "$deadzone:%d#", data);
    if (length <= 0 || length >= (int)sizeof(send_buff)) return false;
    return Motor_Usart_SendArrayBounded(send_buff, (uint16_t)length);
}
...
{
    int length = snprintf((char *)send_buff, sizeof(send_buff), "$mline:%d#", data);
    if (length <= 0 || length >= (int)sizeof(send_buff)) return false;
    return Motor_Usart_SendArrayBounded(send_buff, (uint16_t)length);
}
...
{
    int length = snprintf((char *)send_buff, sizeof(send_buff), "$mphase:%d#", data);
    if (length <= 0 || length >= (int)sizeof(send_buff)) return false;
    return Motor_Usart_SendArrayBounded(send_buff, (uint16_t)length);
}

//设置轮子直径	Send wheel diameter
bool send_wheel_diameter(float data)
{
    int length = snprintf((char *)send_buff, sizeof(send_buff), "$wdiameter:%.3f#", data);
    if (length <= 0 || length >= (int)sizeof(send_buff)) return false;
    return Motor_Usart_SendArrayBounded(send_buff, (uint16_t)length);
}
...
void splitString(char* mystrArray[],char *str, const char *delimiter)
{
    char *token = strtok(str, delimiter); //这是第一次分割，第一个字符值	This is the first split, the first character value
		mystrArray[0] = token; //保留第一次分割字符		Keep the first split character
    int i =1;
...
void Deal_data_real(void)
{
	 ...
	 if ((strncmp("MAll",(char*)g_recv_buff_deal,4)==0))
    {
        ...
        {
            data[i] = g_recv_buff_deal[i+5]; //去除冒号	Remove the colon
        }
				data[length] = '\0';


				char* strArray[10];//指针数组 长度根据分隔符定义  char 1字节   char* 4字节	 Pointer array The length is defined by the split number char 1 byte char* 4 bytes
				char mystr_temp[4][10] = {{0}};
				splitString(strArray,(char*)data, ", ");//按逗号分割	Split by comma
				for (int i = 0; i < 4; i++)
				{
