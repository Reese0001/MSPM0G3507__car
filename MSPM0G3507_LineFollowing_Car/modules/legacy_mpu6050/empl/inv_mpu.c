...
#define MOTION_DRIVER_TARGET_MSP430		//定义驱动部分,使用MSP430驱动(移植到STM32F4) Define the driver part, using the MSP430 driver (ported to STM32F4)
...
//    printf("res = %d\r\n",res);
	...
	{
		res=mpu_set_sensors(INV_XYZ_GYRO|INV_XYZ_ACCEL);//设置需要的传感器 Set up the required sensors
		if(res)return 1;
		res=mpu_configure_fifo(INV_XYZ_GYRO | INV_XYZ_ACCEL);//配置FIFO Setting up FIFO
		if(res)return 2;
...
//得到dmp处理后的数据(注意,此函数需要比较多的栈,移植时请注意)
